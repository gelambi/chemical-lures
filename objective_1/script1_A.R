...
glmm_graph_allbats <- ggplot(fruitbatsp_data, aes(x = treatment, y = prediction_fruitbatsp)) +
  ...
  geom_jitter(data = fruitbatsp_data, aes(x = treatment, y = values, color = bat_species, shape = site), width = 0.05, size = 4) +
...
fruitbats_glmm_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = fruitbats_data, family = poisson)
fruitbats_glmm_nb1_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = fruitbats_data, family = nbinom1(link = "log"))
fruitbats_glmm_nb2_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = fruitbats_data, family = nbinom2(link = "log"))
fruitbats_glmm_hurdle_poisson_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = fruitbats_data, family = truncated_poisson)
fruitbats_glmm_hurdle_bn2_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = fruitbats_data, family=truncated_nbinom2)
fruitbats_glmm_zi_site <- glmmTMB(values ~ treatment + (1|site), zi = ~site, data = fruitbats_data, family = poisson)
fruitbats_glmm_nb1_zi_site <- glmmTMB(values ~ treatment + (1|site), zi = ~site, data = fruitbats_data, family = nbinom1(link = "log"))
fruitbats_glmm_nb2_zi_site <- glmmTMB(values ~ treatment + (1|site), zi = ~site, data = fruitbats_data, family = nbinom2(link = "log"))
fruitbats_glmm_hurdle_poisson_zi_site  <- glmmTMB(values ~ treatment + (1|site), zi = ~site, data = fruitbats_data, family = truncated_poisson)
fruitbats_glmm_hurdle_bn2_zi_site  <- glmmTMB(values ~ treatment + (1|site), zi = ~site, data = fruitbats_data, family=truncated_nbinom2)

# compare all models
check_distribution(fruitbats_glmm) # neg. binomial (zero-infl.) 84%

AICtab(fruitbats_glmm, # poisson
       fruitbats_glmm_nb1, fruitbats_glmm_nb2, # negative binomial
       fruitbats_glmm_zi, # zero inflated poisson
       fruitbats_glmm_nb1_zi, fruitbats_glmm_nb2_zi,  # zero inflated negative binomial
       fruitbats_glmm_hurdle_poisson_zi, # hurdle model, poisson
       fruitbats_glmm_hurdle_bn2_zi, # hurdle model, negative binomial
       fruitbats_glmm_zi_site, # zero inflated poisson
       fruitbats_glmm_nb1_zi_site, fruitbats_glmm_nb2_zi_site,  # zero inflated negative binomial
       fruitbats_glmm_hurdle_poisson_zi_site, # hurdle model, poisson
       fruitbats_glmm_hurdle_bn2_zi_site,  # hurdle model, negative binomial
       base=TRUE, weights=TRUE, logLik=TRUE)
...
artibeus_glmm_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = artibeus_data, family = poisson)
...
artibeus_glmm_nb1_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = artibeus_data, family = nbinom1(link = "log")) 
artibeus_glmm_nb2_zi <- glmmTMB(values ~ treatment + (1|site), zi = ~treatment, data = artibeus_data, family = nbinom2(link = "log")) # Model convergence problem

# poisson
check_overdispersion(artibeus_glmm) # No overdispersion detected.
check_zeroinflation(artibeus_glmm) # Model is overfitting zeros.
check_distribution(artibeus_glmm) # neg. binomial (zero-infl.) 53%
plot(allEffects(artibeus_glmm))

# poisson, zi
check_overdispersion(artibeus_glmm_zi) # No overdispersion detected.
check_zeroinflation(artibeus_glmm_zi) # Model is overfitting zeros.
plot(allEffects(artibeus_glmm))

# negative binomial 1
check_overdispersion(artibeus_glmm_nb1) # No overdispersion detected.
check_zeroinflation(artibeus_glmm_nb1) # Model seems ok, ratio of observed and predicted zeros is within the tolerance range.
plot(allEffects(artibeus_glmm_nb1))

# negative binomial 2
check_overdispersion(artibeus_glmm_nb2) # No overdispersion detected.
check_zeroinflation(artibeus_glmm_nb2) # Model seems ok, ratio of observed and predicted zeros is within the tolerance range.
plot(allEffects(artibeus_glmm_nb2))
...
tab_model(artibeus_glmm) ### tab best model: artibeus_glmm_nb1
...
glmm_graph_artibeus <- ggplot(artibeus_data, aes(x = treatment, y = prediction_artibeus)) +
  ...
  scale_color_viridis(option="D", discrete=TRUE, name= "Sites", labels = c("site 1" = "A", "site 2" = "B", "site 3" = "C", "site 4" = "D", "site 5" = "E", "site 6" = "F")) +
  scale_x_discrete(labels = c("control" = "Control", "lures" = "Chemical lures")) + 
  ylab ("Total number of artibeus spp. bats") +
  xlab ("")
glmm_graph_artibeus
